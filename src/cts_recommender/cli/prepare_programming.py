...
def main():
    ...
    a = p.parse_args()
    _, out = programming_pipeline.run_original_Rdata_programming_pipeline(raw_file=Path(a.rdata), out_file=Path(a.out_processed))
    logger.info(f"Wrote {out}")

    _, out = programming_pipeline.run_enrich_programming_with_movie_metadata_pipeline(processed_file=out, out_file=Path(a.out_enriched))
    logger.info(f"Wrote {out}")
